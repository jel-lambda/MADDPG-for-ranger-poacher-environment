...
def parse_args():
    ...
    parser.add_argument("--save-gifs", action="store_true", default=False, help="save terminal display as a gif instead")
...
def train(arglist):
    with U.single_threaded_session():
        ...
        if arglist.save_gifs:
            assert arglist.display, 'Must use --display to use --save_gifs'
        ...
        while True:
            ...
            if len(episode_rewards) > arglist.num_episodes:
                ...
                break
        if arglist.save_gifs:
            imageio.mimsave(arglist.plots_dir + arglist.exp_name + '_replay.gif', frames, duration=1/5)
            disp.stop()
